    ...
    <div className="min-h-screen bg-slate-50 w-full block">
      ...
      <div className="bg-gradient-to-r from-blue-900 to-indigo-900 text-white py-16 px-4 text-center block w-full">
        <div className="max-w-4xl mx-auto">
          <h1 className="text-3xl md:text-5xl font-extrabold mb-4 tracking-tight">Cari Kost Impianmu di Sini</h1>
          <p className="text-blue-100 text-sm md:text-base mb-8 max-w-xl mx-auto">Temukan hunian kos yang nyaman, strategis, dan sesuai dengan budget kantongmu dengan mudah.</p>
          ...
          <div className="bg-white p-2 rounded-xl shadow-lg max-w-2xl mx-auto flex flex-col md:flex-row gap-2 items-center">
            ...
            <button className="w-full md:w-auto bg-indigo-600 hover:bg-indigo-700 text-white font-bold px-6 py-2.5 rounded-lg transition text-sm shadow-md whitespace-nowrap">Cari Sekarang</button>
          </div>
          ...
          <div className="flex flex-wrap justify-center gap-2 mt-6">
            {["Semua", "Putra", "Putri", "Campur"].map((tipe) => (
              <button
                ...
                className={`px-4 py-1.5 rounded-full text-xs font-semibold transition ${selectedTipe === tipe ? "bg-white text-indigo-900 shadow" : "bg-white/10 hover:bg-white/20 text-white border border-white/20"}`}
              >
                {tipe}
              </button>
            ))}
      ...
      <main className="max-w-7xl mx-auto px-4 py-12 block">
        <div className="flex justify-between items-baseline mb-6 border-b border-slate-200 pb-3">
          <h2 className="text-xl md:text-2xl font-bold text-slate-800">{selectedTipe !== "Semua" ? `Rekomendasi Kost ${selectedTipe}` : "Semua Rekomendasi Kost"}</h2>
          <span className="text-xs md:text-sm text-slate-500 font-medium bg-slate-100 px-2.5 py-1 rounded-full">Menampilkan {filteredKosts.length} kost</span>
        </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-20 bg-white rounded-xl border border-dashed border-slate-200 p-8">
            ...
            <p className="text-slate-400 text-xs max-w-xs mx-auto">Maaf, tidak ada kos-kosan yang cocok dengan kata kunci "{searchQuery}" atau filter tipe tersebut.</p>
            <button
              onClick={() => {
                setSearchQuery("");
      ...
      <section id="contact" className="bg-white py-16 px-4 block w-full border-t border-slate-200">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-10">
            <span className="inline-block text-xs font-bold tracking-widest text-indigo-600 border border-indigo-200 bg-indigo-50 rounded-full px-4 py-1 mb-4">HUBUNGI KAMI</span>
            <h2 className="text-3xl md:text-4xl font-extrabold text-slate-800 leading-tight">
              Ada Pertanyaan? <br className="hidden md:block" />
              Kami Siap Membantu
            </h2>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-10 items-center">
            {/* Kiri: Info Kontak */}
            <div>
              <p className="text-slate-500 text-sm md:text-base mb-8 max-w-md">Tim FindKost siap membantu kamu menemukan kost terbaik. Jangan ragu untuk menghubungi kami kapan saja!</p>
              ...
              <div className="flex gap-3">
                <a
                  href="https://instagram.com/findkost"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex items-center gap-2 border border-slate-200 rounded-full px-4 py-2 text-sm font-semibold text-slate-700 hover:border-indigo-300 hover:text-indigo-600 transition"
                >
                  📷 Instagram
                </a>
                <a
                  href="https://tiktok.com/@findkost"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex items-center gap-2 border border-slate-200 rounded-full px-4 py-2 text-sm font-semibold text-slate-700 hover:border-indigo-300 hover:text-indigo-600 transition"
                >
      ...
      <footer className="bg-slate-900 text-slate-300 py-12 px-4 block w-full">
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-10">
          <div>
            ...
            <p className="text-sm text-slate-400 max-w-xs">Platform tepercaya penyedia info hunian kost terbaik, murah, aman, dan berfasilitas lengkap disekitar UNNES.</p>
          </div>
